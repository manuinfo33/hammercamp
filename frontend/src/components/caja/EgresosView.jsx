        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
            <div className="input-group">
              ...
              <input placeholder="Ej: Alquiler cancha" value={form.concepto} onChange={e => setForm(p => ({ ...p, concepto: e.target.value }))} />
            </div>
            <div className="input-group">
              <label>Monto *</label>
          ...
          <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end', marginTop: '0.5rem' }}>
            ...
            <button type="submit" disabled={loading} style={{ background: 'linear-gradient(135deg,#e07070,#c05050)', color: '#fff', border: 'none' }}>
...
export default function EgresosView({ onTransaccion }) {
  ...
  const fetch = () => {
    setLoading(true);
    const params = new URLSearchParams({ tipo: 'egreso' });
    ...
    <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '10px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          <h2 style={{ fontSize: '1.25rem', fontWeight: 800, color: '#e07070' }}>↓ Egresos</h2>
          {!loading && <span className="badge" style={{ background: 'rgba(224,112,112,0.12)', color: '#e07070', border: '1px solid rgba(224,112,112,0.2)' }}>{formatARS(total)}</span>}
        </div>
        <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
          ...
          <button onClick={() => { setEditing(null); setModal(true); }} style={{ background: 'linear-gradient(135deg,#e07070,#c05050)', color: '#fff', border: 'none' }}>
        ...
        <table>
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={6} style={{ textAlign: 'center', padding: '2.5rem', color: 'var(--text-muted)' }}>No hay egresos registrados</td></tr>
            ) : items.map(item => (
              <tr key={item.id}>
                ...
                <td style={{ color: '#e07070', fontWeight: 700 }}>{formatARS(item.monto)}</td>
                <td>
